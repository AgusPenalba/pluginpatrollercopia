...
class StatisticsService {
    ...
    //Prepara opciones de grupos para dropdowns con información de repositorios
    public function prepareGroupOptions(array $users_by_repo): array {
        $group_options = [];
        ...
        foreach ($users_by_repo as $repo_key => $repo_info) {
            ...
            $group_text = get_string('group', 'mod_pluginpatroller');
            $students_text = get_string('students', 'mod_pluginpatroller');
            $label_text = $group_text . ' ' . $groupIndex . ' — ' . $repo_name . ' (' . count($students) . ' ' . $students_text . ')';
            $group_options[] = [ 'value' => $repo_key, 'text' => $label_text, 'repo_name' => $repo_name, 'group_label' => $group_text . ' ' . $groupIndex ];

            // arrays por estudiantes del repo
            $arrays = $this->chartArraysFromStudents($students);
        ...
        return [
            ...
            'group_chart_map' => $group_chart_map
        ];
    }

    //Prepara datos agrupados de charts (suma por repositorio)
    public function prepareGroupedChartData(array $users_by_repo): array {
        $groups_labels = [];
        $groups_commits = [];
        $groups_lines_added = [];
        ...
        foreach ($users_by_repo as $repo_key => $repo_info) {
            ...
            $students = $repo_info['students'] ?? [];
            $group_text = get_string('group', 'mod_pluginpatroller');
            $groups_labels[] = $group_text . ' ' . $groupIndex;
        ...
        return [
            ...
            'groups_commits' => $groups_commits,
            'groups_lines_added' => $groups_lines_added,
            'groups_lines_deleted' => $groups_lines_deleted,
            'groups_lines_modified' => $groups_lines_modified
        ];
    }

    /**
     * Prepara datos para gráficos con valores por defecto si no hay datos
     */
    public function prepareChartData(array $students_data): array {
        $chartData = [
            'labels' => [],
            'commits' => [],
            'lines' => [],
        ...
        if (!empty($students_data)) {
            $chartArrays = $this->chartArraysFromStudents($students_data);
            $chartData['labels'] = $chartArrays['labels'] ?? [];
            $chartData['commits'] = $chartArrays['commits'] ?? [];
            $chartData['lines'] = $chartArrays['lines'] ?? [];
            $chartData['lines_added'] = $chartArrays['lines_added'] ?? [];
            $chartData['lines_deleted'] = $chartArrays['lines_deleted'] ?? [];
            $chartData['lines_modified'] = $chartArrays['lines_modified'] ?? [];
        }
