...
class my_cron_task extends \core\task\scheduled_task {
    ...
    public function execute() {
		...
		try {
			// 1. Obtener todas las filas de repositorios_data_patroller.
			$repos = $DB->get_records('repositorios_data_patroller', null, '', 'id, id_materia');
			...
			if (!empty($repos)) {
				...
				mtrace("No se encontraron repositorios en la tabla repositorios_data_patroller.");
			}
		} catch (Exception $e) {
		...
		if (curl_errno($ch_commits)) {
			...
			foreach ($commiter_array as $commiter_name => $commiter_info) {
				foreach ($students as $student) {
					if ($commiter_name == $student->usuario_github && $student->id_repo == $repo_id) {
						$DB->update_record(
							'usuarios_data_patroller',
							[
								'id' => $student->id,
	...
	function get_students_by_repoid_and_courseid($repo_id, $courseid){
		...
		$resultado = $DB->get_records('usuarios_data_patroller', ['id_repo' => $repo_id, 'id_materia' => $courseid]);
